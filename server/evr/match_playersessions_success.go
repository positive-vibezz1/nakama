...
func (m *LobbyEntrantsV3) String() string {
	return fmt.Sprintf("%T(unk0=%d, xp_id=%v, entrant_id=%s, team_index=%d, unk1=%d, unk2=%d)",
		m, m.Unk0, m.XPID, m.EntrantID, m.TeamIndex, m.Unk1, m.Unk2)
}
...
func (m LobbyEntrantsV2) String() string {
	return fmt.Sprintf("%T(unk0=%d, xp_id=%v, entrant_id=%s)", m, m.Unk0, m.XPID, m.EntrantID)
}
